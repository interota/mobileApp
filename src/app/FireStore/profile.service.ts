...
export class ProfileService extends FireStoreDocumentService<Profile> {
  ...
  async getUserDetails(): Promise<Profile> {
   ...
   async getProfileByUserId(userId: string): Promise<Profile> {
    let profile = null;
      ...
      let t =  await this.firestore
        .collection(Profile.getCollectionName(), (ref) =>
          ref.where('UserId', '==', userId)
        )
        .get().toPromise();

          t.docs.forEach((d) => (profile = Profile.fromFireStore(d)))
  ...
  async fillFromJson(json: any[]) {
    for (let i = 0; i < this.participant.length; i++) {
      let profile = this.participant[i];
      try
      {

        let usr =
        await this.fireService.angularFireAuthentication.signInWithEmailAndPassword(
          profile.Email,
          '123456789'
          )
        ...
        catch(e)
        {
          ...
          try{
            ...
            let usr= await this.fireService.angularFireAuthentication.createUserWithEmailAndPassword(
              profile.Email,
              '123456789'
              )
              profile.UserId = usr.user.uid;
              this.obj_after.push(profile);
            }
            catch(e)
            {
    ...
    this.obj_after.forEach(async (profile) => {
      await this.add(
        new Profile(
          '',
          profile.FullName,
